...
bool TrajectoryGenerator::callback_set_mode(test_package::StringSrv::Request &req, test_package::StringSrv::Response &resp)
{
...
void TrajectoryGenerator::spin()
{
    ...
    // srv.request.yaw = 0.0;
    // while(!(home_client.call(srv) && srv.response.success))
    // {
    //     ROS_INFO("Failed to set home position, reattemping in 250ms");
    //     ros::Duration(0.25).sleep();
    // }
    // ROS_INFO("Home position set");
    ...
    while(ros::ok())
    {
        ...
        test_package::DroneTrajectory traj_msg;
        ...
        if(yaw_tracking_enable)
        {
            // traj_msg.yaw = atan2(traj_msg.velocity.y, traj_msg.velocity.x);
            // if(traj_msg.velocity.x < 0)
            // {
            //     traj_msg.yaw += M_PI;
            // }
            // else if(traj_msg.velocity.x == 0 && traj_msg.velocity.y != 0)
            // {
            //     traj_msg.yaw = (traj_msg.velocity.y > 0) ? M_PI_2 : -M_PI_2;
            // }
        }
        else
        {
